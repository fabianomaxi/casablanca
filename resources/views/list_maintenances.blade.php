            ...
            <div class="main-content">
                ...
                <div class="page-content">
                    <div class="container-fluid">
                        ...
                        <div class="card">
                                    <div class="card-body">
                                        <h4 class="card-title">Listagem de Manutenção</h4>
                                        <p class="card-title-desc">Gestão de Manutenção</p>

                                        <div class="w-50">
                                            <form name="drmBusca" id="frmNovo" action="/maintenance" method="post">
                                            @csrf
                                            </form>
                                            <form name="drmBusca" id="frmbusca" action="/maintenances" method="get">
                                            @csrf
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                 ...
                                 @foreach( $data as $d )
                                    ...
                                    <tr>
                                       <td contenteditable="true">{{date('d/m/Y H:i:s',strtotime($d->date))}}</td>
                                       <td contenteditable="true">{{$situations[$d->situation]}}</td>
                                       <td contenteditable="true">{{$d->description}}</td>
                                       <td contenteditable="true">{{$users[$d->id_user]}}</td>
                                       <td contenteditable="true">{{$products[$d->id_products]}}</td>
                                       <td contenteditable="true">{{@$prodSerie[$d->id_products]}}</td>

                                       <td>
                                          <span class="table-remove"><button type="button" onclick="javascript:window.location='/maintenances/{{$d->id_maintenance}}'"
                                             class="btn btn-primary btn-rounded waves-effect waves-light">Editar</button></span>

                                             <span class="table-remove"><button type="button"
                                                class="btn btn-danger btn-rounded waves-effect waves-light" onclick="javascript:del( {{$d->id_maintenance}} )">Deletar</button></span>
                                       </td>
                                    </tr>
