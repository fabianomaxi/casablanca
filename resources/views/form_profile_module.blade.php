                     ...
                     <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                           <div class="iq-header-title">
                              <h4 class="card-title">Módulos permitidos</h4>
                           </div>
                        </div>
                        <div class="iq-card-body">
                           
                           <form  action="/profile_modules" method="post" id="frmCadastro">
                              <input type="hidden" name="id" id="id" value="{{@$data->id_profile_module}}">
                                @csrf
                            <div class="form-row">
                              <div class="col">
                             ...
                             <label for="email">usuarios</label>
                                <select class="form-control" id="exampleFormControlSelect1" required name="id_user">
                                    ...
                                    <option value="7" {{(@$data->id_user == 7) ? 'selected' : ''}}>Fabiano</option>
                                    <option value="16" {{(@$data->id_user == 16) ? 'selected' : ''}}>João</option>
                                 </select>
                             </div>

                             <div class="col">
                             <label for="email">Perfil</label>
                                <select class="form-control" id="exampleFormControlSelect1" required name="id_profile">
                                    <option value="">Perfil</option>
                                    <option value="1" {{(@$data->id_profile == 1) ? 'selected' : ''}}>ADM</option>
                                    <option value="12" {{(@$data->id_profile == 12) ? 'selected' : ''}}>Financeiro</option>
                                 </select>
                             </div>

                             <div class="col">
                             <label for="email">Modelos</label>
                                <select class="form-control" id="exampleFormControlSelect1" required name="id_modules">
                                    <option value="">Modulo</option>
                                    <option value="2" {{(@$data->id_modules == 2) ? 'selected' : ''}}>Perfil</option>
                                    <option value="6" {{(@$data->id_modules == 6) ? 'selected' : ''}}>Modulos</option>
                                 </select>
                             </div>
