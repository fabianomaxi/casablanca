                    ...
                    <div class="container-fluid">
                        ...
                        <div class="card">
                                    <div class="card-body">
                                        ...
                                        <p class="card-title-desc">Gestão de Locais de Estoque</p>

                                        <div class="w-50">
                                            <form name="drmBusca" id="frmNovo" action="/stock_location" method="post">
                                            @csrf
                                            </form>
                                            <form name="drmBusca" id="frmbusca" action="/stock_locations" method="get">
                                            @csrf
                            ...
                            <div class="col-lg-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <thead>
                                    <tr>
                                       <th>Nome do Local de estoque</th>
                                       <th>Tipo</th>
                                       <th>Logradouro</th>
                                       <th>Usuário</th>
                                 ...
                                 @foreach( $data as $d )
                                    @php
                                       $tipos = ['1' => 'Padrão','2'=>'Móvel','3'=>'Externo'] ;
                                    @endphp   
                                    <tr>
                                       <td contenteditable="true">{{$d->name}}</td>
                                       <td contenteditable="true">{{@$tipos[$d->tipo]}}</td>
                                       <td contenteditable="true">{{$d->logradouro}}</td>
                                       <td contenteditable="true">{{$d->user->name}}</td>
                                       <td contenteditable="true">{{$d->license_plate}}</td>

                                       <td>
                                          <span class="table-remove"><button type="button" onclick="javascript:window.location='/stock_locations/{{$d->id_stock_locations}}'"
                                             class="btn btn-primary btn-rounded waves-effect waves-light">Editar</button></span>

                                             <span class="table-remove"><button type="button"
                                                class="btn btn-danger btn-rounded waves-effect waves-light" onclick="javascript:del( {{$d->id_stock_locations}} )">Deletar</button></span>
                                       </td>
                                    </tr>
